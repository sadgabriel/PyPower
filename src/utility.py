...
def move_rect_by_alignment(rect, pos: tuple[int], alignment: tuple[int]):
    if alignment[0] == -1:
        ...
        raise ValueError(
            "alignment must be a tuple of -1, 0, or 1 whose length is 2 like (-1, 0), (1, -1), or (1, 1)"
        )
...
class DirectionFinder:
    ...
    def find(
        self, fiducial_sprite: pygame.sprite.DirtySprite, direction, fiducial_angle=5
    ):
        ...
        if direction == direction.UP:
            ...
            raise ValueError(
                "direction must be UP, DOWN, LEFT or RIGHT in definition.py"
            )
        ...
        for sprite in self._sprites:
            ...
            if (
                ...
                and fiducial_sprite.rect.centery <= sprite.rect.centery
            ):
                print("continued")  #######################
                continue
            elif (
                ...
                print(
                    "continued", fiducial_sprite.rect.centery, sprite.rect.centery
                )  #######################
                continue
            elif (
                direction == direction.LEFT
                and fiducial_sprite.rect.centerx <= sprite.rect.centerx
            ):
                print("continued")  #######################
                continue
            elif (
        ...
        if candidates:
            if direction == direction.UP:
                return max(
                    ...
                    key=lambda sprite: (
                        sprite.rect.centery
                        - abs(sprite.rect.centerx - fiducial_sprite.rect.centerx)
                    ),
                )
            elif direction == direction.DOWN:
                return min(
                ...
                return max(
                    ...
                    key=lambda sprite: (
                        sprite.rect.centerx
                        - abs(sprite.rect.centery - fiducial_sprite.rect.centery)
                    ),
                )
            elif direction == direction.RIGHT:
                return min(
...
def find_vector(point1: tuple[int], point2: tuple[int]):
    """Return vector of point1 to point2"""
    return (point2[0] - point1[0], point2[1] - point1[1])


def find_angle(vector: tuple[int]):
